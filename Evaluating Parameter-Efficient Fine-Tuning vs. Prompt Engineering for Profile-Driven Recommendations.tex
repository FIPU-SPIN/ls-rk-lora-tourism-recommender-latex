...
\newcommand{\orcidauthorA}{0009-0004-4210-5756} % RK
\newcommand{\orcidauthorB}{0009-0002-1965-7822}%LS
\newcommand{\orcidauthorC}{0000-0002-3015-1024}%SBS
\newcommand{\orcidauthorE}{0000-0002-5964-245X}%IL
%\newcommand{\orcidauthorB}{0000-0000-0000-000X} % Add \orcidB{} behind the author's name

% Authors, for the paper (add full first names)
\Author{Ratomir Karlović $^{1}$\orcidA{}, Luka Sever $^{1}$\orcidB{}, Sandi Baressi Šegota $^{1}$\orcidC{} and Ivan Lorencin $^{1}$\orcidE{}}
...
\abstract{Large language models (LLMs) show significant promise for personalized recommendation systems, yet identifying the most effective adaptation strategy for structured data remains a challenge. This study evaluates the performance trade-offs between parameter-efficient fine-tuning (PEFT) and in-context learning approaches for personalized item recommendation. We utilized explicit user profiles, encompassing specific likes and dislikes, alongside a comprehensive JSON-formatted item catalog. Two distinct adaptation paradigms were compared: PEFT, specifically utilizing Low-Rank Adaptation (LoRA) and Infused Adapter by Inhibiting and Amplifying Inner Activations (IA$^3$), against robust prompt engineering strategies optimizing input instructions without altering model weights. The approaches were systematically evaluated using standard ranking metrics, including Normalized Discounted Cumulative Gain (NDCG@K) and Hit Ratio (HR@K), alongside computational efficiency measures. Our findings demonstrate that while few-shot prompt engineering establishes a competitive baseline, PEFT methods, particularly LoRA, achieve superior alignment with nuanced user preferences, yielding significant improvements in ranking accuracy. Furthermore, PEFT effectively mitigates context-window limitations, reducing inference latency compared to extensive prompt demonstrations. Consequently, we conclude that adapter-based fine-tuning provides more reliable and scalable personalized recommendations from structured catalogs than prompt engineering alone, successfully bridging raw LLM reasoning with strict domain-specific requirements.}
